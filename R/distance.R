...
parkruns2=cross_join(parkruns %>% dplyr::select(-short), parkruns %>% dplyr::select(-short))

dist=numeric(nrow(parkruns2))
for(i in 1:nrow(parkruns2)){
  dist[i]=distm (c(parkruns2$lon.x[i], parkruns2$lat.x[i]), c(parkruns2$lon.y[i], parkruns2$lat.y[i]), fun = distHaversine)/1000
  if(i%%10000==0)
  {
